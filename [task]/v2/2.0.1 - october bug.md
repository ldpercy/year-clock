...
* Noticed incorrect positioning for the month-day hand only in the month of October
* And then only for certain years or ranges of years
* October was reporting 30 instead of 31 days for the problematic years
* Turns out to be related to Australian Eastern daylight savings changeover which subtracts an hour from October, making the 'floor' function drop down a day
* The other DST change in April was unaffected as it adds an hour
* Changed floor to 'round' in `dayDifference` as a temporary fix
* Proper fix will come with [changeover to temporal](<2.Δ📅 - changeover to temporal.md>)
* Removed the redundant '.class' infix from theme module filenames
...
* Conversion to Temporal
* More work on testing code
