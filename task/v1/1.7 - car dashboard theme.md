...
2025-08-08		1.7.1	Follow-ups done
```
...
Season events
-------------
This one is going to bite.
The current season handling is flawed enough as it is, but the summer year-spanning hack means the summer range is currently incomplete.

I need to rethink this whole thing, and hopefully get the hemispheres right as well.
I should probably the take the emojis out of the array and find another way to add/map them.

Probably need to have two different season arrays:
* One for the actual year's seasons with proper boundaries at the year crossing for s.summer/n.winter
	This one will have five items per year
* One with the year crossing hack, only as needed for certain clocks
	This one will have four items per year

The car dashboard theme will actually need both arrays: the 4-element version for the temperature-wheel, and the 5 element version for the season warning light.

I solidly doubt I can so this cleanly first time around, will just do an ugly functional version first, then try to see how I can simplify it.

Have a first go at adding a 'proper' season array - this one fixes season the warning lamp.
It's pretty ugly at the moment.
Still need to figure out a way - some sort of semi-acceptable way - of changing the season ids based on hemisphere.

I've hacked in the quickest hackiest version I could think of.
It's horrible, but it works.


Wrapup for follow-ups
---------------------

* Figured out emoji problem in ff
* Added star cutout to body path
* Improved layering of clock - now have backing, face and body
* Add warning lights with face cutouts
* moved day and date text, applied a little stretching/squishing
* getDateLabel improved, getyearlabel removed
* Add a few initial event getter methods - pretty hackish right now, just something to start with
* [Added hemisphere switch](<1.7.1 - hemisphere switch.md>) - can now swap between N and S hemisphere - seasons will swap on clocks that show it
* Added extra season array method to make it clear when the circle wrapping hack is needed
* Added some long awaited extra labels for the lightning clock
